...
class BasicVisualizer(Visualizer):
    ...
    def render(self, trajectories, batch=None):
        ...
        if batch is None:
            ...
            rewards = rewards[0]
            self._render_batch(states, actions, rewards)

    def _render_batch(self, states, actions, rewards, horizon=None):
        if horizon is None:
            ...
            for t in range(horizon):
                ...
                r = rewards[t]
                self._render_timestep(t, s, a, r)
            self._render_round_end(rewards)

    def _render_timestep(self, t, s, a, r):
        print("============================")
        print("TIME = {}".format(t))
        print("============================")
